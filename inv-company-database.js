...
const fetchBatchFromSupabase = async () => {
    ...
    allFetchedData = data.map(row => ({
        name: row.name?.trim(),
        content: row.inv_tax_indo_content?.trim()
    }));
...
const importContentForSelectedName = (clickedGoogleSheetDataName) => {
    ...
    if (clickedGoogleSheetDataName.style.backgroundColor === 'rgb(0, 155, 0)') {
        ...
        const invNumElement = document.querySelector("#current_used_inv_tax_p_id");
        let revNumElement = document.querySelector("#current_used_rev_number_p_id");
        let revNumValue = document.querySelector("#store_supabase_current_inv_tax_rev_number_id");


        if (!revNumElement) {
            // Create the element since it doesn't exist
            revNumElement = document.createElement("p");
            revNumElement.id = "current_used_rev_number_p_id";
        ...
        // Set the text
        revNumElement.innerText = `-(${revNumValue.innerText})`;
